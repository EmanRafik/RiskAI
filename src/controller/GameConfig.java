...
public class GameConfig {
	...
	// Color is 0 -> red, 1 -> blue
	public static void updateTerritory(int territoryId, int numberOfTroops, int color) {
		// blue case
		if (color == 1) {
			territoriesBlue.get(territoryId).setVisible(true);
			territoriesRed.get(territoryId).setVisible(false);
			armyLabels.get(territoryId).setText("" + numberOfTroops);
		}
		// red case
		if (color == 0) {
			territoriesBlue.get(territoryId).setVisible(false);
			territoriesRed.get(territoryId).setVisible(true);
			armyLabels.get(territoryId).setText("" + numberOfTroops);
		}
	}

	// updates an array of territories
	// Assumes that a dummy is placed in first location---
	public static void updateTerritories(Territory[] territories) {
		for (int i = 1; i < territories.length; i++) {
			Territory currentTerritory = territories[i];
			updateTerritory(currentTerritory.getTerritoryID(), currentTerritory.getTroopsCount(),
					currentTerritory.getHolderID());
		}
	}
}
